...
`go-ruby-confd/confd` reimplements Ruby's `confd` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-confd/confd` directly, with no Ruby runtime;
...
Because `confd` is pure Go with cgo disabled, it also compiles to **WebAssembly**
